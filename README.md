...
- 输入框文件按钮可附加本机文件/文件夹：服务端目录浏览弹窗选中真实路径，智能体即可对该路径读写；气泡内展示附件卡片，弹窗内可查看/撤销本会话授权
...
### 设置
- 文件沙箱配置：主工作区目录 + 白名单目录，支持服务端目录浏览选择，保存后对智能体立即生效
...
```
src/
├── api/            后端接口封装（http / user / agent / conversation / modelPreset / settings / fs）
