...
- 单聊与群聊：群聊支持自由讨论、编排者协作、成员 @ 与置顶；群内左键点头像即可 @ 对方；群标题「（N人）」可点击查看成员列表（编排者徽标并排第一，普通群聊与协作任务群一致）；回复进行中发送按钮变为「停止」可随时掐断（协作/讨论在顶部横幅终止）
- 微信接入：侧边栏手机图标扫码连接微信 iLink Bot，手机微信直接与智能体单聊；消息实时落库到「微信ClawBot」会话（小龙虾头像、桌面端只读），可随时切换处理智能体，图片/文件/视频与语音转写均可处理，回复流式实时可见
...
- 历史会话：删除/解散/开始新会话的聊天自动归档到「历史会话」，可按日期分组（分组可折叠，默认仅展开最新一组）、搜索、查看完整记录、继续聊天或彻底删除；定时任务删除后整批归档于此，以「定时任务」标签区分
