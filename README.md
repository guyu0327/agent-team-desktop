...
- 实时语音识别配置：讯飞开放平台（xfyun.cn）应用的 APPID / APIKey / APISecret（需开通「语音听写」，每天 500 次免费），保存后按住麦克风即可实时转写；全部留空保存则清除配置
...
```
src/
├── api/            后端接口封装（http / user / agent / conversation / modelPreset / settings / fs / asr）
