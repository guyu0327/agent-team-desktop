...
私有化部署的桌面客户端：Electron 窗口内置 [agent-team-web] 前端，并自动拉起 [agent-team-server] 后端（SQLite 单文件数据库），零外部依赖，开箱即用。
...
├── preload.js            向渲染进程注入 window.agentTeam（apiBase + token）
...
| 命令 | 模式 | 行为 |
| --- | --- | --- |
| `npm start` | 桌面模式 | 立即显示启动等待页 → 自动分配空闲端口 → spawn `java -jar resources/server/agent-team-server.jar`（SQLite，数据落在用户目录）→ 健康检查 `/api/user` → 切换到内置前端；关窗自动结束后端进程 |
...
- [ ] 应用图标与代码签名（当前使用 Electron 默认图标，未签名）
- [ ] 自动更新（electron-updater，按需）
- [ ] macOS 打包（dmg/app，暂缓）

## 相关仓库

- [agent-team-server]：Spring Boot 后端（编排、会话、消息、SSE）
- [agent-team-web]：Vue 3 前端（类微信界面）
